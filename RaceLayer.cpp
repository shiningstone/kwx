	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			auto StartSelfGame=CallFunc::create([=](){
				if(!player[1]->get_parter()->get_ting_status())
			...
			//auto seqFunc=Sequence::create(CallFunc::create([=](){myframe->removeChildByTag(TUOGUAN_CANCEL_BUTTON,true);}),CallFunc::create(this,callfunc_selector(RaceLayer::waitfor_MyTouchShowCard)),
				//CallFunc::create(this,callfunc_selector(RaceLayer::waitfor_MyShowCardInstruct)),NULL);
		}
		break;
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			TuoGuanCancel->addTouchEventListener(CC_CALLBACK_2(RaceLayer::tuoGuanCancelFunc,this));
			...
			if(myframe->getChildByTag(QI_REMIND_ACT_TAG_ID)&&ifMyActionHint)
			{
				...
				if(g_server==1)
				{
					if(ifQiangGangAsk)
					{
						...
						auto curFunc=CCCallFunc::create(this,callfunc_selector(RaceLayer::call_distribute_card));
...
void RaceLayer::BackChooseEnsure(Ref* pSender,ui::Widget::TouchEventType type)
{
	switch (type)
	{
	case cocos2d::ui::Widget::TouchEventType::BEGAN:
...
void RaceLayer::BackChooseCancel(Ref* pSender,ui::Widget::TouchEventType type)
{
	switch (type)
	{
	case cocos2d::ui::Widget::TouchEventType::BEGAN:
...
void RaceLayer::BackChoose()
{
	...
	ensureBut->addTouchEventListener(CC_CALLBACK_2(RaceLayer::BackChooseEnsure,this));
	...
	BackBut->addTouchEventListener(CC_CALLBACK_2(RaceLayer::BackChooseCancel,this));
...
void RaceLayer::back_callback(Ref* pSender,cocos2d::ui::Widget::TouchEventType type)
{
	auto curButton=(Button*)pSender;
...
void RaceLayer::create_race()
{
	...
	robotButton->addTouchEventListener(CC_CALLBACK_2(RaceLayer::robot_callback,this));
	...
	backButton->addTouchEventListener(CC_CALLBACK_2(RaceLayer::back_callback,this));
	...
	{	
		if(i!=1)
		{
			...
			player[i]->set_player_id(noOfRobot);
		...
		{
			player[i]->set_player_id(17);
			nameOfPlayer="MyNickName";
			photoOfPlayer="MyPhoto";
			propertyOfPlayer="MyProperty";
			SexOfPlayer="MySex";
		}
		player[i]->set_nick_name(userDefault->getStringForKey(nameOfPlayer.c_str()));
		player[i]->set_photo(userDefault->getStringForKey(photoOfPlayer.c_str()));
		player[i]->set_property(userDefault->getIntegerForKey(propertyOfPlayer.c_str()));
		player[i]->set_sex(userDefault->getStringForKey(SexOfPlayer.c_str()));
	}
	sprintf(bufferCash,"Language%d",0);
	std::string languageKind=bufferCash;
	for(int i=0;i<3;i++)
	{
		player[i]->set_language(userDefault->getStringForKey(languageKind.c_str()));
		player[i]->get_sex(playerSex[i]);
		player[i]->get_language(playerLanguage[i]);
	}
	...
	StartButton->addTouchEventListener(CC_CALLBACK_2(RaceLayer::start_touchCallBack,this));
	this->addChild(StartButton,2,START_GAME_TAG_ID);
	std::string buffer;
	int score;
	for(int i=0;i<3;i++)//更新玩家信息
	{
		player[i]->get_photo(buffer);
		update_headimage(i,buffer);
		player[i]->get_nick_name(buffer);
		update_nickname(i,buffer);
		player[i]->get_property(score);
...
void RaceLayer::set_user_default()
{
...
void RaceLayer::get_user_default()
{
	int i;
	auto userDefault=UserDefault::getInstance();
...
void RaceLayer::update_score(int direction,int score)
{
...
void RaceLayer::update_nickname(int direction,std::string str_Nick)
{		
	if(direction==0)
	{
		if(!left_player_bkg)
...
void RaceLayer::update_headimage(int direction,std::string head_photo)
{
...
void RaceLayer::GoldNumInsert(int GoldWinner,int Gold_kind,int who_give)
{
	...
	{
		auto my_score=player[1]->get_parter()->get_card_score();
		auto robot2_score=player[2]->get_parter()->get_card_score();
		auto robot0_score=player[0]->get_parter()->get_card_score();
...
bool RaceLayer::resource_prepare()
{
...
bool RaceLayer::get_ready_status(int direction)
{
	return ready_status[direction];
}

void RaceLayer::update_clock(bool visible,int time,int direction)
{		
...
void RaceLayer::startParticleSystem(float delta)
{
	auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			curButton->setTouchEnabled(false);
			auto callFunc1=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::peng_tip_effect));
			curButton->_ID=1;
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			if(ifQiangGangAsk)
			{
				...
				auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
				auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::hu_tip_effect));
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			{
				auto callFunc1=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::an_gang_tip_effect));
				curButton->runAction(Sequence::create(ScaleTo::create(0.1,1),callFunc1,NULL));
			}
			else if(action_todo&a_MING_GANG)
			{
				auto callFunc1=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::ming_gang_tip_effect));
				curButton->runAction(Sequence::create(ScaleTo::create(0.1,1),callFunc1,NULL));
			}
		}
		break;
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			curButton->setTouchEnabled(false);
			curButton->_ID=1;
			auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::qi_tip_effect));
			curButton->runAction(Sequence::create(ScaleTo::create(0.1,1),callFunc,NULL));
		}
		break;
	case cocos2d::ui::Widget::TouchEventType::CANCELED:
...
void RaceLayer::delete_NnnecessaryResource()
{
}
...
BezierTo* RaceLayer::BizerMove1(outCardList* outCard,Vec2 location)
{
...
BezierTo* RaceLayer::BizerMove2(outCardList* outCard,Vec2 location,int time)
{
	ccBezierConfig config;
	if((outCard->length-1)<6)
	{
...
BezierTo* RaceLayer::OthersBizerMove(int no,outCardList* outCard)
{
	ccBezierConfig config;
	auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
	if(no==2)
	{
		int lenForPlayerTwo=player[2]->get_parter()->get_card_list()->len;
	...
	{
		auto l_list_len=player[no]->get_parter()->get_card_list()->len-1;
...
void RaceLayer::collect_resources(HAH *res,CARD_KIND target1[],CARD_KIND target2[],int *len1,int *len2)
{
	...
	player[k]->get_parter()->get_hu_cards(target1,len1);
	k=(cur_player+2)%3;
	player[k]->get_parter()->get_hu_cards(target2,len2);
	for(int i=player[cur_player]->get_parter()->get_card_list()->atcvie_place;i<player[cur_player]->get_parter()->get_card_list()->len;i++)
	{
		int time=res->list[player[cur_player]->get_parter()->get_card_list()->data[i].kind].same_times++;
		res->list[player[cur_player]->get_parter()->get_card_list()->data[i].kind].place[time]=i;
	}
	/*init hu target*/
	if(player[cur_player]->get_parter()->get_ting_status()!=1)
		player[cur_player]->init_target(&res->target,*len1,*len2);
...
void RaceLayer::update_outcard(Node *myframe,Vec2 location,int time)
{
	...
	if(action_todo==a_MING&&player[cur_player]->get_parter()->get_ting_status()==0)
	{
		_eventDispatcher->removeEventListenersForTarget(myframe,true);
	...
	auto callFunc0=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_in_river_list));
	//auto callFunc1=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
	auto callFunc2=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::waitfor_response));
	auto sim=Sequence::create(callFunc0,CCCallFunc::create([=]()
	{
		player[1]->get_parter()->action(g_server,a_JUMP);
		...
		else
		{
			int no=1;
			auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
			unsigned char ting_status=player[no]->get_parter()->get_ting_status();
			if(ting_status==1)
...
void RaceLayer::waitfor_MyShowCardInstruct()
{
	...
	if(g_server==0)
	{
		if(ifTuoGuan)
		{
			int last_one=player[cur_player]->get_parter()->get_card_list()->len-1;
		...
		else if(player[cur_player]->get_parter()->get_ting_status()!=0)
		{
...
void RaceLayer::waitfor_MyTouchShowCard()//正常情况下的出牌监听（非托管和明牌）
{
	...
	if(player[1]->get_parter()->get_ting_status()==0&&!ifTuoGuan)
	{
		...
		listener->onTouchBegan=[=](Touch* touch, Event* event)
		{
			...
			auto list=player[1]->get_parter()->get_card_list();
			...
			{
				while(myframe->getChildByTag(TING_SING_BAR)&&(!player[1]->get_parter()->get_ting_status()))
			...
			for(int i=list->atcvie_place;i<list->len;i++)
			{
							...
							else if(k==i)
							{
								...
								if(k==list->len-1)
								{
									...
									if(loopCard->getScaleX()==1)
									{
										...
										if(action_todo==a_MING&&ifMingTime&&(!player[1]->get_parter()->get_ting_status()))
										{
									...
									else if(action_todo==a_MING&&ifMingTime&&(!player[1]->get_parter()->get_ting_status()))
									{
										if(!myframe->getChildByTag(TING_SING_BAR))
										{
											auto appearPoint=loopCard->getPosition();
				...
				else if(touch->getLocation().y>visibleSize.height*0.173)
				{
					if(myframe->getChildByTag(CHOOSE_CARD_TAG_ID)==NULL&&ifChosed==true)
					{
						...
						int single_click_chose_card=player[1]->get_parter()->get_card_list()->data[cardInList].kind;
...
void RaceLayer::waitfor_ShowCardWithoutTouch()
{
	...
	if(cur_player!=1)
	{
		...
		if(player[cur_player]->get_parter()->get_ting_status()==0)
		{
			...
			if(s_res->hu_nums>=6&&action_todo==a_MING&&player[cur_player]->get_parter()->get_ting_status()==0)
			{
				RototHandOutIndex=player[cur_player]->get_parter()->get_card_list()->data[index].kind;
		...
		auto show_card_kind=Sprite::createWithTexture(g_small_card_kind[g_show_card]->getTexture());
		float p_x,p_y;
		auto RobotList=player[cur_player]->get_parter()->get_card_list();
		...
		if(cur_player==2)
		{
			...
			int curListLen=player[2]->get_parter()->get_card_list()->len;
		...
		{
			player[cur_player]->get_parter()->action(g_server,a_JUMP);
...
void RaceLayer::peng_tip_effect(Node *psender)//效果逻辑分离
{
	...
	{
		auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
		auto peng_action=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::peng_update));
		auto callFunc_update_list=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
		auto actcheckagain=CCCallFunc::create([=](){
			action_todo=player[cur_player]->get_parter()->ActiontodoCheckAgain();
			if(no==1)
				waitfor_myaction(no);
			else
	...
	else
	{
		if(myframe->getChildByTag(PENG_EFFECT_NODE_ID))
		{
			...
			//myframe->_ID=1;
			//myframe->runAction(CallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list)));
		}
		...
		auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_ActionEffect));
		//auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
		auto peng_action=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::peng_update));
...
void RaceLayer::angang_update(Node *psender)
{
	...
	if(action_todo&a_AN_GANG)
		...
		player[no]->get_parter()->action(g_server,a_SHOU_GANG);
}
void RaceLayer::an_gang_tip_effect(Node *psender)
{
	...
	{
		auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
		auto angang_action=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::angang_update));
		auto callFunc_update_list=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
		auto callFunc_distribute_card=CCCallFunc::create(this,callfunc_selector(RaceLayer::call_distribute_card));
	...
	else 
	{
		...
		int gang[4];
		int ii;
		int tingFlag=player[cur_player]->get_parter()->get_ting_status();
		if(tingFlag==0)
		{
...
void RaceLayer::QiangGangHuJudge()
{
	...
	unsigned char action1=player[no]->get_parter()->hand_in(g_show_card,g_server,curTingStatus,false,a_QIANG_GANG,continue_gang_times,isGangHua);
	int no1=(cur_player+2)%3;
	unsigned char action2=player[no1]->get_parter()->hand_in(g_show_card,g_server,curTingStatus,false,a_QIANG_GANG,continue_gang_times,isGangHua);
	//action1=a_HU;
	//action2=a_HU;
	if((action1&a_HU)&&(action2&a_HU))//双响
	{
		...
		if((no!=1&&no1!=1)||((no==1||no1==1)&&player[1]->get_parter()->get_ting_status()==1))
		{
			hu_effect_tip(3);
			distribute_event(DOUBLE_HU_WITH_ME,NULL);
		}
		else if((no==1||no1==1)&&player[1]->get_parter()->get_ting_status()==0)
		{
			ifDoubleAsk=true;
			if(no==1)
			{
...
void RaceLayer::ming_gang_tip_effect(Node *psender)
{
	...
	{
		int outcard_place=player[cur_player]->get_parter()->getOutCardList()->length;
		player[cur_player]->get_parter()->getOutCardList()->getCard(kind_out_card,outcard_place);
		player[cur_player]->get_parter()->getOutCardList()->deleteItem();
		if(myframe->getChildByTag(HAND_OUT_CARDS_TAG_ID+cur_player*25+outcard_place-1))
		{
			myframe->removeChildByTag(HAND_OUT_CARDS_TAG_ID+cur_player*25+outcard_place-1);
	...
	if(no!=1)
	{
		...
		auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
		auto minggang_action=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::minggang_update));
		auto callFunc_update_list=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
		CallFunc* dis_action;
		if(g_server==0)
		{
			QiangGangTarget=no;
			dis_action=CCCallFunc::create(this,callfunc_selector(RaceLayer::QiangGangHuJudge));
		}
		else
			dis_action=CCCallFunc::create(this,callfunc_selector(RaceLayer::call_distribute_card));
...
void RaceLayer::qi_tip_effect(Node *psender)
{
	...
	auto seq=Sequence::create(ScaleTo::create(0,1),spawn,NULL);
	auto shadeAction=TargetedAction::create(shade_act,seq);
	auto DeleteActTip=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_ActionRemind));
	...
	if(myframe->getChildByTag(QI_REMIND_ACT_BKG_TAG_ID)!=NULL&&no==1)
	{
		...
		else
		{
			if(ifQiangGangAsk)//抢杠胡请求
			{
				...
				auto DistributeFunc=CCCallFunc::create(this,callfunc_selector(RaceLayer::call_distribute_card));
...
void RaceLayer::update_residue_TingCards(int no)
{
	...
	player[no]->get_parter()->get_hu_NumForEveryCard(hu_NumForEveryCard);//张数
	player[no]->get_parter()->get_hu_residueForEvery(hu_residueForEvery);//剩余牌数
	//player[no]->get_parter()->get_huTiemsForEveryOne(huTiemsForEveryOne);//番型
	//player[no]->get_parter()->get_hu_cards(hu_cards);//胡哪几张牌
...
void RaceLayer::OtherTingHintBar(int curNo,int CardPlace)
{
	...
	int hu_NumForEveryCard[MAX_HANDIN_NUM];//胡张数
	CARD_KIND hu_cards[MAX_HANDIN_NUM][9];//胡哪几张牌
	player[curNo]->get_parter()->get_hu_NumForEveryCard(hu_NumForEveryCard);//张数
	//player[curNo]->get_parter()->get_hu_residueForEvery(hu_residueForEvery);//剩余牌数
	//player[curNo]->get_parter()->get_huTiemsForEveryOne(huTiemsForEveryOne);//番型
	player[curNo]->get_parter()->get_hu_cards(hu_cards);//胡哪几张牌
...
void RaceLayer::tingHintCreate(Point curPos,int CardPlace)
{
	...
	int hu_NumForEveryCard[MAX_HANDIN_NUM];//胡张数
	CARD_KIND hu_cards[MAX_HANDIN_NUM][9];//胡哪几张牌
	player[1]->get_parter()->get_hu_NumForEveryCard(hu_NumForEveryCard);//张数
	player[1]->get_parter()->get_hu_residueForEvery(hu_residueForEvery);//剩余牌数
	player[1]->get_parter()->get_huTiemsForEveryOne(huTiemsForEveryOne);//番型
	player[1]->get_parter()->get_hu_cards(hu_cards);//胡哪几张牌
...
void RaceLayer::card_list_update(int no)
{
		...
		if(list->data[i].kind!=ck_NOT_DEFINED )
		{
			if(no==0)
			{
				if(list->data[i].status==c_FREE)
				{
					...
					else if(ting_flag!=1&&player[1]->get_parter()->get_ting_status()==1)
					{
				...
				{
					if(list->data[i].kind==list->data[i+1].kind&&list->data[i].kind!=list->data[i+2].kind&&ting_flag!=1&&player[1]->get_parter()->get_ting_status()==1)
						p_list[i]=Sprite::createWithTexture(g_left_right_peng_out->getTexture());
					else
						p_list[i]=Sprite::createWithTexture(g_left_right_angang->getTexture());
					p_list[i]->setAnchorPoint(Vec2(0.3f,1.0f));
				...
				if(list->data[i].status==c_FREE)
				{
					if(ting_flag==1||player[1]->get_parter()->get_ting_status()==1)
					{
						Sprite* s_card;
						if(ting_flag==1)
				...
				else if(list->data[i].status==c_PENG)
				{
					...
					s_card->setScale(0.9);
					p_list[i]->addChild(s_card);
					if(player[1]->get_parter()->get_ting_status()==1||ting_flag==1)
					{
			...
			if(no==1&&action_todo==a_MING&&player[1]->get_parter()->get_ting_status()!=1&&list->data[i].can_play==cps_YES)
	...
	if(no==1&&myframe->getChildByTag(TING_SING_BUTTON))//||(no!=1&&player[no]->get_parter()->get_ting_status()))
...
void RaceLayer::update_card_list(Node *psender)
{
	int no=psender->_ID;
	auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			player[1]->get_parter()->action(g_server,a_KOU);
			auto ming_indexesCur=player[1]->get_parter()->ming_check();
			player[1]->get_parter()->set_ming_indexes(ming_indexesCur);
			auto buttonAction=TargetedAction::create(curButton,buttonact);
			auto callFunc0=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));//????
			auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
			auto callFunc2=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::ming_tip_effect));
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
			auto list=player[1]->get_parter()->get_card_list();
			player[1]->get_parter()->action(g_server,a_KOU_CANCEL);
			action_todo=a_JUMP;
			auto callFunc2=CallFunc::create([=](){
				action_todo=player[1]->get_parter()->ActiontodoCheckAgain();
				waitfor_myaction(1);
			});
			player[1]->get_parter()->MingCancel();
			player[1]->get_parter()->set_ming_indexes(0);
			player[1]->get_parter()->set_ting_status(0);
			auto callFunc0=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
			auto callFunc1=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
			auto seq1=Sequence::create(ButtonAction,callFunc0,callFunc1,callFunc2,NULL);
			myframe->_ID=cur_player;
	...
	for(a=list->atcvie_place;a<list->len;a++)
	{
		...
		{	
			if(list->data[b].kind==list->data[a].kind)
			{
				...
				if(num==3)
				{
					if(player[no]->get_parter()->judge_kou_cards(list->data[a].kind,no,RototHandOutIndex))
					{
...
void RaceLayer::ming_kou_Choose(int no)
{
	...
	if(no==1)
	{
		...
		ChooseCancel->addTouchEventListener(CC_CALLBACK_2(RaceLayer::KouCancle,this));
		...
		ChooseEnsure->addTouchEventListener(CC_CALLBACK_2(RaceLayer::KouEnsure,this));
		...
		int a;
		CARD_ARRAY *list=player[1]->get_parter()->get_card_list();
		Sprite *p_list[MAX_HANDIN_NUM];
		...
		KouListener->onTouchEnded=[=](Touch* touch, Event* event)
		{
			...
			for(a=0;a<Kou_kindLen;a++)
			{
				...
				{
					if(player[1]->get_parter()->judge_kou_cards(kouCards_kind[a].kind,no,RototHandOutIndex))
					{
	...
	else
	{
		...
		for(a=0;a<Kou_kindLen;a++)
		{
			if(player[no]->get_parter()->judge_kou_cards(kouCards_kind[a].kind,no,RototHandOutIndex))
			{
				for(b=0;b<3;b++)
				{
...
void RaceLayer::ming_tip_effect(Node *psender)
{
	...
	if(psender->_ID==1)
	{
		...
		MingCancel->addTouchEventListener(CC_CALLBACK_2(RaceLayer::MingCancle,this));
		...
		auto callFunc0=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
		auto callFunc2=CCCallFunc::create(this,callfunc_selector(RaceLayer::waitfor_MyTouchShowCard));
	...
	else if(psender->_ID!=1)
	{
		...
		auto callFunc2=CCCallFunc::create(this,callfunc_selector(RaceLayer::waitfor_ShowCardWithoutTouch));
		myframe->_ID=curNo;
		myframe->runAction(callFunc2);
	}
	/*active_place_indexes 是按照bit 位来只是哪些张牌可以出，如果bit0==1 表示左手起的第一张可以出*/
	//update_card_list(cur_player-1);
	//scheduleOnce(schedule_selector(RaceLayer::waitfor_ShowCardWithoutTouch),0.3);
	//scheduleOnce(schedule_selector(RaceLayer::delete_act_tip),0.3);
}
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			_eventDispatcher->removeEventListenersForTarget(myframe,true);
			auto callfunc=CallFuncN::create(this,callfuncN_selector(RaceLayer::ming_tip_effect));
			auto callFunc0=CCCallFunc::create(this,callfunc_selector(RaceLayer::delete_act_tip));
...
void RaceLayer::waitfor_myaction(int no)
{
	...
	if(action_todo!=a_JUMP)
	{
		...
		myact_qi->_ID=no;
		myact_qi->addTouchEventListener(CC_CALLBACK_2(RaceLayer::qi_callback,this));
	...
	{	
		...
		myact_hu->_ID=no;
		myact_hu->addTouchEventListener(CC_CALLBACK_2(RaceLayer::hu_callback,this));
	...
	if(action_todo&a_MING)//明牌
	{
		...
		myact_ming->addTouchEventListener(CC_CALLBACK_2(RaceLayer::ming_callback,this));
	...
	if(action_todo&a_AN_GANG || action_todo&a_MING_GANG||action_todo&a_SHOU_GANG)//暗杠和明杠,手杠
	{
		...
		myact_gang->addTouchEventListener(CC_CALLBACK_2(RaceLayer::gang_callback,this));
	...
	{		
		...
		myact_peng->addTouchEventListener(CC_CALLBACK_2(RaceLayer::peng_callback,this));
...
void RaceLayer::waitfor_otheraction(int no)
{
	...
	{
		auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::an_gang_tip_effect));
		myframe->runAction(callFunc);
	}
	else if(action_todo&a_MING_GANG)
	{
		auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::ming_gang_tip_effect));
		myframe->runAction(callFunc);
	}
	else if(action_todo&a_MING)
	{
		auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::ming_tip_effect));
		myframe->runAction(callFunc);
	}
	else if(action_todo&a_PENG)
	{
		auto callFunc=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::peng_tip_effect));
		myframe->runAction(callFunc);
	}
...
void RaceLayer::waitfor_response(Node* sender)
{
	...
	if(g_server==0)
	{
		...
		if(last_actionSource==sender->_ID&&continue_gang_times!=0)
			...
			continue_gang_times=0;
		action_todo=player[sender->_ID]->get_parter()->hand_in(g_show_card,g_server,curTingStatus,is_last_one,last_action_WithGold,continue_gang_times,isGangHua);
		if(sender->_ID==1)
		{
			auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
			float x=base_point[1].x;
			float y=base_point[1].y;
			if(player[1]->get_parter()->get_ting_status()==1)
			{
				x += myframe->getChildByTag(HAND_IN_CARDS_TAG_ID+cur_player*20+(player[1]->get_parter()->get_card_list()->len-2))->getPosition().x+30;
				y += 60+13;
			}
			else
			{
		...
		if(player[sender->_ID]->get_parter()->get_role_type()==SINGLE_BOARD_ROBOT)
		{
	...
	else
	{
		...
		unsigned char action1=player[no]->get_parter()->hand_in(g_show_card,g_server,curTingStatus,is_last_one,last_action_WithGold,continue_gang_times,isGangHua);
		if(no==1&&ifTuoGuan)
		{
			if(player[1]->get_parter()->get_ting_status()==1&&(action1&a_HU))
				action1=a_HU;
			else
				action1=a_JUMP;
		}
		if(player[no]->get_parter()->get_role_type()==SINGLE_BOARD_ROBOT)
		{
...
void RaceLayer::distribute_card_effect()
{
	...
	unsigned char ting_flag=player[cur_player]->get_parter()->get_ting_status();
	if(cur_player==1)
	{
	...
	//auto callFunc=CallFuncN::create(this,callfuncN_selector(RaceLayer::update_card_list));
	auto callFunc1=CCCallFuncN::create(this,callfuncN_selector(RaceLayer::waitfor_response));
	Sequence* seq2;
	myframe->_ID=cur_player;
	if(cur_player!=1)
...
void RaceLayer::distribute_card()
{
	...
	auto _noonewinListener = EventListenerCustom::create(NOONE_WIN_EVENT_TYPE, [this](EventCustom * event){
		...
		auto myframe=this->getChildByTag(GAME_BKG_TAG_ID);
		set_winner_no(4);
		auto callfunc=CallFunc::create(this,callfunc_selector(RaceLayer::showall));
	...
	case cocos2d::ui::Widget::TouchEventType::ENDED:
		{
			...
			auto startGame=CallFunc::create(this,callfunc_selector(RaceLayer::start_callback));
...
void RaceLayer::effect_Distribute_Card(int zhuang)
{
	...
	auto ListOfNo1=player[1]->get_parter()->get_card_list();
	...
	int RightLen=player[2]->get_parter()->get_card_list()->len+1;
	for(a=0;a<4;a++)
	{
	...
	action_todo=a_JUMP;
	myframe->runAction(Sequence::create(seq,update_lists_spa,updateClock,CallFunc::create(this,callfunc_selector(RaceLayer::start_dealCardsDelete)),
		CCCallFunc::create([=](){
...
void RaceLayer::AccountShows(LayerColor* BarOfPlayer,int no)
{
	...
	player[no]->get_property(PropertyOfPlayer);
	player[no]->get_photo(PhotoOfPlayer);
	player[no]->get_nick_name(NickNameOfPlayer);
	char buffer[80];
...
void RaceLayer::raceAccount(float delta)
{
	...
	PlayOnceAgin->addTouchEventListener(CC_CALLBACK_2(RaceLayer::restart_touchCallBack,this));
...
void RaceLayer::hu_tip_effect(Node *psender)
{
	int no=psender->_ID;
	//if(no==3)
	//{
...
//int RaceLayer::get_cur_player_no()
//{
//	return cur_player;
//}
//void RaceLayer::set_cur_player_no(int no)
//{
//	cur_player=no;
//}
void RaceLayer::init_role(Role *s_p[],Raction *p_parter[])
{
	...
	{
		player[j]=s_p[j];
		player[j]->set_parter(p_parter[j]);
	}
}
void RaceLayer::set_reserved_num(int no)
{
	total_reserved_card_num=no;
}
int RaceLayer::get_reserved_num()
{
	return total_reserved_card_num;
}

void RaceLayer::set_cards_sequence(const int list[])
{
	for(int i=0;i<INIT_CARDS_NUM;i++)
		card_seq[i]=list[i];
}

void RaceLayer::set_aims_sequence(const int p_aim[])
{
	for(int i=0;i<3;i++)
		aim[i]=p_aim[i];
}

void RaceLayer::updatePropertyImmediate(int GoldNum[3])
{
	for(int a=0;a<3;a++)
	{
		int PropretyOfPlayer;
		player[a]->get_property(PropretyOfPlayer);
		PropretyOfPlayer+=GoldNum[a];
		player[a]->set_property(PropretyOfPlayer);
		update_score(a,PropretyOfPlayer);
		update_UserDefault(a);
	}
}
void RaceLayer::update_UserDefault(int no)
{
	auto userDefault=UserDefault::getInstance();
	int propertyOfPlayer;
	player[no]->get_property(propertyOfPlayer);
	...
	int PlayerForId;
	player[no]->get_player_id(PlayerForId);
