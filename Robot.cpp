...
int Robot::Robot_check_pickup_card(CARD_KIND kind,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
	int j,k;
	for(j=0;j<len1;j++)
		if(list1[j]==kind)
...
int Robot::river_reserved_card(HAH *hash_table,int card)
{
	int num=0;
...
int Robot::Robot_check_card_stable(HAH *card_array,CARD_KIND card)
{
...
int Robot::Robot_picup_single_for_samecolor(int color,HAH *card_array,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
	int chose_place=-1;
	int offset=color*9;
...
int Robot::Robot_pickup_for_sevencouples(HAH *card_array,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{	
	int chose_place=-1;
...
int Robot::Robot_pickup_for_fourpeng(HAH *card_array,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
	int chose_place=-1;
...
int Robot::Robot_pickup_for_lowwin(HAH *card_array,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
	int chose_place=-1;
	int index1=card_array->river_len-1;
	int index2=index1-1;
...
Raction* Robot::get_parter()
{
	return parter;
}

void Robot::set_parter(Raction* p_parter)
{
	parter=p_parter;
}
int Robot::Robot_pickup_single(HAH *card_array,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
	int chose_place=-1;
...
void Robot::set_robot_hu_target(ROBOT_TARGET par_target)
{
	g_target=par_target;
}

ROBOT_TARGET Robot::get_robot_hu_target()
{
	return g_target;
}

int Robot::chose_card(HAH *pres,int reseved,CARD_KIND list1[],CARD_KIND list2[],int len1,int len2)
{
...
void Robot::init_target(ROBOT_TARGET *target,int hu_len1,int hu_len2)
{
	CARD_ARRAY *list=parter->get_card_list();
	int couples=0;
	int color;
...
void Robot::set_player_id(int player_id)
{
	idOfPlayer=player_id;
}
bool Robot::get_player_id(int & player_id)
{
	player_id=idOfPlayer;
	return true;
}
void Robot::set_photo(const std::string pho)
{
	photo=pho;
}
bool Robot::get_photo(std::string & pho)
{
	pho=photo;
	return true;
}
void Robot::set_nick_name(const std::string name)
{
	nickName=name;
}
bool Robot::get_nick_name(std::string & name)
{
	name=nickName;
	return true;
}

unsigned char Robot::init(int card_array[],int len,int aim)
{
	return parter->init(card_array,len,aim);
}

void Robot::set_property(int pro)
{
	property=pro;
}

bool Robot::get_property(int & pro)
{
	pro=property;
	return true;
}
void Robot::set_sex(std::string sex) 
{
	playerSex=sex;

}
bool Robot::get_sex(std::string & sex)
{
	sex=playerSex;
	return true;
}
void Robot::set_language(std::string language)
{
	playerLanguage=language;
}
bool Robot::get_language(std::string & language)
{
