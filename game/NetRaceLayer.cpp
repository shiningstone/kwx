    	...
    	case cocos2d::ui::Widget::TouchEventType::ENDED:
    		{
    			...
    			if(_roundManager->_isQiangGangAsking) {
    				...
    				auto myframe = this->getChildByTag(GAME_BKG_TAG_ID);
    				auto shadeAction = _gEffect->Shade(myframe->getChildByTag(HU_REMIND_ACT_TAG_ID));
...
TargetedAction *NetRaceLayer::ShowBigMing(Node *myframe) {
    ...
    damingFont->setPosition(_coordinate->PositionOfMingSign());
...
bool NetRaceLayer::_CardTouchBegan(Touch* touch, Event* event) {
    ...
    if( touch->getLocation().y > visibleSize.height*0.173 ) {
        ...
        const auto cardSize = _texture->RectSize(FREE_CARD);
...
void NetRaceLayer::_CardTouchMove(Touch* touch, Event* event) {
    ...
    auto cardSize = _texture->RectSize(FREE_CARD);
    float x1,y1;
...
void NetRaceLayer::waitfor_ShowCardWithoutTouch()
{
    ...
    *******************/
	...
	smallCard->runAction(RotateTo::create(0,_coordinate->RotateAngleOfOutcard(curPlayer)));
	...
	CallFunc* showAndHideOutcardNotice = CallFunc::create([=](){
		...
		cardFrame->setAnchorPoint(_coordinate->AnchorOfOutcard(curPlayer));
		cardFrame->setPosition(_coordinate->PositionOfOutcard(curPlayer));
	...
	Sprite   * cardOut = _texture->Create(LR_OUT_CARD);
        ...
        && _roundManager->_actionToDo==a_MING && !_roundManager->IsTing(_roundManager->_curPlayer))
	{
		...
		auto simple_seq = simple_tip_effect( _coordinate->PositionOfActSign(_roundManager->_curPlayer),"daming.png" );
...
void NetRaceLayer::PengEffect(Node *psender)//效果逻辑分离
{
	...
	if(no!=1) {
		myframe->runAction(Sequence::create( 
                            Spawn::create(
                                _voice->SpeakAction(PENG,
                                    _roundManager->_players[_roundManager->_curPlayer]->get_sex()),
                                simple_tip_effect( _coordinate->PositionOfActSign(_roundManager->_curPlayer),"peng.png" ),NULL), 
                            hideOutcard, 
        ...
        ****************/
		auto cardPeng = _texture->Create(PENG_CARD);
        ...
        ****************/
        ...
        auto EmptyCard=_texture->Create(FREE_CARD);
        ...
        ****************/
		...
		Spawn *simple_seq = simple_tip_effect( _coordinate->PositionOfActSign(_roundManager->_curPlayer),"peng.png" );
...
void NetRaceLayer::an_gang_tip_effect(Node *psender)
{
    ...
    if(no!=1) {
		myframe->runAction(Sequence::create(
            Spawn::create(
                _voice->SpeakAction(GANG,
                    _roundManager->_players[_roundManager->_curPlayer]->get_sex()),
                simple_tip_effect(_coordinate->PositionOfActSign(_roundManager->_curPlayer),"gang.png"),NULL), CallFunc::create([=](){
        ...
        **********************/
        Size FreeCardSize = _texture->RectSize(FREE_CARD);
        ...
        **********************/
        ...
        auto GangCardSize = _texture->RectSize(AN_GANG_CARD);;
        ...
        **********************/
		...
		AnGangEffectNode->runAction( 
    		Sequence::create(
                hideReminder,
                Spawn::create(
                    simple_tip_effect(_coordinate->PositionOfActSign(_roundManager->_curPlayer),"gang.png"),
                    _voice->SpeakAction(GANG,
                        _roundManager->_players[_roundManager->_curPlayer]->get_sex()),
                    moveFreeCards,
                    Spawn::create(
...
void NetRaceLayer::ming_gang_tip_effect(Node *psender)
{
    ...
    if(no!=1){
		...
		Spawn *simple_seq=simple_tip_effect(_coordinate->PositionOfActSign(_roundManager->_curPlayer),"gang.png");///墨迹等。。。update_list_seq 最后处理，转换cur_player
        ...
        **********************/
        ...
        int gang[3];           /* used store index of gang cards in the hand in list */
		...
		if(MyCardChoosedNum!=-1) {
			...
			if(!_roundManager->_isCardFromOthers) {
				if(MyCardChoosedNum>gang[2])
					MyCardChoosedNum+=1;
			} else {
				if(MyCardChoosedNum>gang[2])
					MyCardChoosedNum+=1;
				else if(MyCardChoosedNum==gang[0]||MyCardChoosedNum==gang[1]||MyCardChoosedNum==gang[2]) {
					ifZeroPointTwo=1;
					MyCardChoosedNum=-1;
				}
				else if(MyCardChoosedNum<gang[0])
        ...
        **********************/
		auto Pengsize = _texture->RectSize(PENG_CARD);
        ...
        **********************/
		...
		auto show_card = _texture->Create(PENG_CARD);
		auto cardPengSize = show_card->getTextureRect().size;

        _AttachKindTexture(show_card,GangCard.kind);
        
		show_card->setAnchorPoint(Vec2(0,0));
		if(!_roundManager->_isCardFromOthers) {
			show_card->setPosition(_coordinate->OrigPositionOfGangCard(0,cardPengSize));
		} else {
		...
		if(_roundManager->_isCardFromOthers) {
			...
			gangCardsMotion[0] = TargetedAction::create(s_curOutCard,Sequence::create(
                    DelayTime::create(0.42),
                    ScaleTo::create(0,0.6),
                    MoveTo::create(0.12,_coordinate->OrigPositionOfGangCard(0,cardPengSize)),
                    ScaleTo::create(0,0),NULL));
            gangCard0Motion = Sequence::create(
                gangCardsMotion[0],
                TargetedAction::create(show_card,Sequence::create(
                    ScaleTo::create(0,1),
                    MoveTo::create(0.12,_coordinate->MiddlePositionOfGangCard(0,cardPengSize)),
    				MoveTo::create(0.12,_coordinate->DestPositionOfGangCard(0,cardPengSize)),NULL)),NULL);
		}
        ...
        **********************/
        ...
        if(!_roundManager->_isCardFromOthers) {
            _CreateMingGangCardInHandMotion(gangCardInHandMotion,gang,GangCard.kind);
		} else {
		...
		if(list->data[i].kind!=ck_NOT_DEFINED )
		{
			if(no==0)
			{
				if(list->data[i].status==c_FREE)
				{
					...
					{
						p_list[i]=_texture->Create(LR_AN_GANG_CARD);
						p_list[i]->setAnchorPoint(Vec2(0.3f,1.0f));
						p_list[i]->setPosition(Vec2(x-10,y-8));
					}
					else
					{
						p_list[i]=_texture->Create(L_IN_CARD);
			...
			else if(no==1)
			{
				...
				{
					if(ting_flag==1)
						p_list[i]=_texture->Create(MING_CARD);
					else
						p_list[i]=_texture->Create(FREE_CARD);
				}
				else if(list->data[i].status==c_PENG||list->data[i].status==c_MING_GANG)
				{
					p_list[i]=_texture->Create(PENG_CARD);
				}
				else if(list->data[i].status==c_AN_GANG)
				{
					if(list->data[i].kind==list->data[i+1].kind && list->data[i].kind!=list->data[i+2].kind)
						...
						p_list[i]=_texture->Create(AN_GANG_CARD);
				}
				...
				if(list->data[i].status==c_FREE)
				{
					...
					if(ting_flag==1||(_roundManager->_actionToDo==a_MING&&ting_flag==0))
					{
						...
						{
							auto MingCps_yesMask=_texture->Create(MING_BKG);
						...
						{
							Sprite* mask;
							mask=_texture->Create(MING_MASK_CARD);
				...
				else if(list->data[i].status==c_MING_KOU)
				{
					if(ting_flag!=1)
					{
						auto KouSign=_texture->Create(MING_KOU_CARD);
			...
			else if(no==2)
			{
				if(list->data[i].status==c_FREE)
				{
					...
					{
						p_list[i]=_texture->Create(R_IN_CARD);
...
Node *NetRaceLayer::_NonKouMask(Sprite *card) {
    auto mask = _texture->Create(MING_MASK_CARD);
        ...
        for(a=0;a<Kou_kindLen;a++) {
            for(b=0;b<3;b++) {
                ...
                if(cards->data[KouCardsPlace[a][b]].status==c_MING_KOU) {
                    auto MingKouMark=_texture->Create(MING_KOU_CARD);
                    ...
                    auto KouNo=_texture->Create(MING_MASK_CARD);
...
void NetRaceLayer::distribute_card_effect()
{
	...
	if(_roundManager->_curPlayer==1)
	{
		list_last_one=_texture->Create(FREE_CARD);
	...
	{
		list_last_one=_texture->Create(R_IN_CARD);
	...
	{
		list_last_one=_texture->Create(L_IN_CARD);
...
void NetRaceLayer::effect_Distribute_Card(int zhuang)
{
	...
	float x,y;
	//自己
	auto Mecard_in=_texture->Create(IN_CARD);	
	auto Mehand_in=_texture->Create(FREE_CARD);
	...
	for (a=0;a<4;a++)
	{
		meHandCard[a]=_texture->Create(IN_CARD);
	...
	//右手
	auto RightHandIn=_texture->Create(R_IN_CARD);
	auto RightCardIn=_texture->Create(LR_IN_CARD);
	...
	for (a=0;a<4;a++)
	{
		RobotHandCard[a]=_texture->Create(LR_IN_CARD);
	...
	for(a=0;a<4;a++)
	{
		...
		{	
			...
			lastTwo[0]=_texture->Create(R_IN_CARD);
			...
			if(zhuang==2)
			{
				...
				lastTwo[1]=_texture->Create(R_IN_CARD);
	...
	for (a=0;a<4;a++)
	{
		LeftRobotHandCard[a]=_texture->Create(LR_IN_CARD);
	...
	for(a=0;a<4;a++)
	{
		...
		{	
			...
			lastTwo[0]=_texture->Create(L_IN_CARD);
			...
			if(zhuang==0)
			{
				...
				lastTwo[1]=_texture->Create(L_IN_CARD);
...
void NetRaceLayer::AccountShows(LayerColor* BarOfPlayer,int no)
{
	...
	CARD_ARRAY *list=_roundManager->_players[no]->get_parter()->get_card_list();
	auto tempCard=_texture->Create(PENG_CARD);
	float x=posOfCards.x;
	float y=posOfCards.y-tempCard->getTextureRect().size.height/2;
	for(int i=0;i<list->len;i++)
	{
		show_card_list[i]=_texture->Create(PENG_CARD);
...
void NetRaceLayer::hu_effect_tip(int no)
{
	...
	if(no<3) {
        ...
        if(no!=1)
		{
			Spawn *simple_seq=simple_tip_effect(_coordinate->PositionOfActSign(_roundManager->_curPlayer),"dahu.png");
			auto GoldAccount=CallFunc::create([=](){
	...
	else if(no==3)//两个对家双响
	{
		...
		auto _doublehucallListener = EventListenerCustom::create(DOUBLE_HU_WITH_ME, [this](EventCustom * event){
			...
			auto HuFontNo=simple_tip_effect( _coordinate->PositionOfActSign((_roundManager->_curPlayer+1)%3),"dahu.png");
			auto HuFontNo1=simple_tip_effect( _coordinate->PositionOfActSign((_roundManager->_curPlayer+2)%3),"dahu.png");
...
void NetRaceLayer::GuiJinBiShow(int dir, int gold) {
	...
	number->setAnchorPoint(_coordinate->AnchorOfNumber(dir));
	number->setOpacity(0);
	number->setPosition(_coordinate->PositionOfNumber(dir));
	this->addChild(number,6,GOLD_NUM_INSERT_NUMBER);

    int xoffset = number->getContentSize().width;
    
	auto minus=Sprite::create("-.png");
	minus->setAnchorPoint(_coordinate->AnchorOfSign(dir));
	minus->setPosition(_coordinate->PositionOfSign(dir,xoffset));
	minus->setOpacity(0);
	this->addChild(minus,6,GOLD_NUM_INSERT_JIANHAO);

	auto plus=Sprite::create("+.png");
	plus->setAnchorPoint(_coordinate->AnchorOfSign(dir));
	plus->setPosition(_coordinate->PositionOfSign(dir,xoffset));
	plus->setOpacity(0);
	this->addChild(plus,6,GOLD_NUM_INSERT_JIAHAO);

	auto jinbi=Sprite::create("jinbi-game.png");
	jinbi->setAnchorPoint(_coordinate->AnchorOfGold(dir));
	jinbi->setOpacity(0);
	jinbi->setPosition(_coordinate->PositionOfGold(dir,xoffset));
	...
	jinbi->runAction(Sequence::create(
        ...
        MoveTo::create(1.5,_coordinate->DestOfGold(dir,xoffset)),NULL),
        ScaleTo::create(0,0),NULL));
    
	number->runAction(Sequence::create(
        DelayTime::create(0),
        Spawn::create(FadeIn::create(1.5),
        MoveTo::create(1.5,_coordinate->DestOfNumber(dir)),NULL),
        ScaleTo::create(0,0),NULL));

    symbol->runAction(Sequence::create(
        DelayTime::create(0),
        Spawn::create(FadeIn::create(1.5),
        MoveTo::create(1.5,_coordinate->DestOfSign(dir,xoffset)),NULL),
        ScaleTo::create(0,0),NULL));
}
...
void NetRaceLayer::_CreateGangCardsMotion(TargetedAction *motions[4]) {
    auto myframe = this->getChildByTag(GAME_BKG_TAG_ID);
    for (int i=0; i<4; i++ ) {
        auto GangCard = _texture->Create(AN_GANG_CARD);
        Size GangCardSize = GangCard->getTextureRect().size;
        GangCard->setAnchorPoint(Vec2(0,0));       
        GangCard->setScale(0);
        
        GangCard->setPosition(_coordinate->OrigPositionOfGangCard(i,GangCardSize));
        myframe->addChild(GangCard,30,SINGLE_ACTION_EFFECT_TAG_ID+i);

        motions[i] = TargetedAction::create(GangCard,Sequence::create(
            DelayTime::create(0.42),
            ScaleTo::create(0,1),
            MoveTo::create(0.12,_coordinate->MiddlePositionOfGangCard(i,GangCardSize)),
            MoveTo::create(0.12,_coordinate->DestPositionOfGangCard(i,GangCardSize)),NULL));
    }
}
...
void NetRaceLayer::_CreateMingGangCardsMotion(TargetedAction *mostions[4],CARD_KIND kind) {
    auto myframe = this->getChildByTag(GAME_BKG_TAG_ID);
    for (int i=1;i<4;i++) {
        auto card = _texture->Create(PENG_CARD);
        Size size = card->getTextureRect().size;
        _AttachKindTexture(card,kind);

        card->setAnchorPoint(Vec2(0,0));
        card->setScale(0);
        card->setPosition(_coordinate->OrigPositionOfGangCard(i,size));
        ...
        mostions[i] = TargetedAction::create(card,Sequence::create(
            ...
            ScaleTo::create(0,1),
            DelayTime::create(0.12),
            MoveTo::create(0.12,_coordinate->MiddlePositionOfGangCard(1,size)),
            MoveTo::create(0.12,_coordinate->DestPositionOfGangCard(1,size)),NULL));
    }
}
...
void NetRaceLayer::_CreateGangCardInHandMotion(TargetedAction *motions[4],int cardInHand[4],CARD_KIND kind) {
    ...
    for (int i=0;i<4;i++ ) {
        ...
        auto GangCard=_texture->Create(FREE_CARD);//gang1
        ...
        motions[i] = TargetedAction::create(GangCard,Sequence::create(
            DelayTime::create(0.18),Spawn::create(
                ScaleTo::create(0,0.6),
                MoveTo::create(0,_coordinate->MiddlePositionOfGangCardInHand(i,OldPos,FreeCardSize)),NULL),
            MoveTo::create(0.18,_coordinate->DestPositionOfGangCardInHand(i,GangCardSize)),
            DelayTime::create(0.06),
            ScaleTo::create(0,0),NULL));
    }
}
...
void NetRaceLayer::_CreateMingGangCardInHandMotion(TargetedAction *motions[3], int idxInHand[3], CARD_KIND kind) {
    ...
    for (int i=0;i<3;i++) {
        ...
        Sprite* card = _texture->Create(PENG_CARD);
        card->setAnchorPoint(Vec2(0,0));
        ...
        motions[i] = TargetedAction::create(card,Sequence::create(
            DelayTime::create(delayTime),
            MoveTo::create(0.18,_coordinate->PositionOfMingGangCard((i+1),card->getTextureRect().size)),
            DelayTime::create(0.06),
            ScaleTo::create(0,0),NULL));
    }
}
...
void NetRaceLayer::_CreateFreeCard(Sprite *cards[3], int idxInHand[3], CARD_KIND kind) {
    ...
    for(int i=0;i<3;i++) {
        ...
        cards[i]=_texture->Create(FREE_CARD);//gang[0]
...
void NetRaceLayer::_CreateBackgroundElementMotion(TargetedAction *motions[5],int gangType) {
    auto myframe = this->getChildByTag(GAME_BKG_TAG_ID);
    Size GangCardSize = _texture->RectSize(AN_GANG_CARD);;
...
****************************************************/
    ...
    void NetRaceLayer::_GuiJinBiShowMe(int gold) {
        ...
        sprintf(str,"%d",abs(gold));
        auto number=LabelAtlas::create(std::string(str),"fonts/moneyMessage.png",28,39,'0');
        number->setAnchorPoint(_coordinate->AnchorOfNumber(1));
        number->setOpacity(0);
        number->setPosition(_coordinate->PositionOfNumber(1));
        this->addChild(number,6,GOLD_NUM_INSERT_NUMBER);
    
        auto minus = Sprite::create("-.png");
        minus->setAnchorPoint(_coordinate->AnchorOfSign(1));
        minus->setPosition(_coordinate->PositionOfSign(1));
        minus->setOpacity(0);
        this->addChild(minus,6,GOLD_NUM_INSERT_JIANHAO);
    
        auto plus=Sprite::create("+.png");
        plus->setAnchorPoint(_coordinate->AnchorOfSign(1));
        plus->setPosition(_coordinate->PositionOfSign(1));
        plus->setOpacity(0);
        this->addChild(plus,6,GOLD_NUM_INSERT_JIAHAO);
        
        auto jinbi=Sprite::create("jinbi-game.png");
        jinbi->setAnchorPoint(_coordinate->AnchorOfGold(1));
        jinbi->setOpacity(0);
        jinbi->setPosition(_coordinate->PositionOfGold(1));
        this->addChild(jinbi,6,GOLD_NUM_INSERT_JINBI);
        ...
        if( gold!=0 ) {
            jinbi->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfGold(1)),NULL),
                ScaleTo::create(0,0),NULL));
            number->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfNumber(1)),NULL),
                ScaleTo::create(0,0),NULL));
            if(gold>0) {
                plus->runAction(Sequence::create(
                    DelayTime::create(0),
                    Spawn::create(FadeIn::create(1.5),
                    MoveTo::create(1.5,_coordinate->DestOfSign(1)),NULL),
                    ScaleTo::create(0,0),NULL));
            } else {
    ...
    void NetRaceLayer::_GuiJinBiShowLeft(int gold) {
        ...
        sprintf(str,"%d",abs(gold));
        auto number=LabelAtlas::create(std::string(str),"fonts/moneyMessage.png",28,39,'0');
        number->setAnchorPoint(_coordinate->AnchorOfNumber(0));
        number->setOpacity(0);
        number->setPosition(_coordinate->PositionOfNumber(0));
        this->addChild(number,6,GOLD_NUM_INSERT_NUMBER);
    
        auto minus=Sprite::create("-.png");
        minus->setAnchorPoint(_coordinate->AnchorOfSign(0));
        minus->setPosition(_coordinate->PositionOfSign(0));
        minus->setOpacity(0);
        this->addChild(minus,6,GOLD_NUM_INSERT_JIANHAO);
    
        auto plus=Sprite::create("+.png");
        plus->setAnchorPoint(_coordinate->AnchorOfSign(0));
        plus->setPosition(_coordinate->PositionOfSign(0));
        plus->setOpacity(0);
        this->addChild(plus,6,GOLD_NUM_INSERT_JIAHAO);
    
        auto jinbi=Sprite::create("jinbi-game.png");
        jinbi->setAnchorPoint(_coordinate->AnchorOfGold(0));
        jinbi->setOpacity(0);
        jinbi->setPosition(_coordinate->PositionOfGold(0));
        this->addChild(jinbi,6,GOLD_NUM_INSERT_JINBI);
        ...
        if( gold!=0 ) {
            jinbi->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfGold(0)),NULL),
                ScaleTo::create(0,0),NULL));
            number->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfNumber(0)),NULL),
                ScaleTo::create(0,0),NULL));
            if(gold>0) {
                plus->runAction(Sequence::create(
                    DelayTime::create(0),
                    Spawn::create(FadeIn::create(1.5),
                    MoveTo::create(1.5,_coordinate->DestOfSign(0)),NULL),
                    ScaleTo::create(0,0),NULL));
            } else {
                minus->runAction(Sequence::create(
                    DelayTime::create(0),
    ...
    void NetRaceLayer::_GuiJinBiShowRight(int gold) {
        ...
        sprintf(str,"%d",abs(gold));
        auto number=LabelAtlas::create(std::string(str),"fonts/moneyMessage.png",28,39,'0');
        number->setAnchorPoint(_coordinate->AnchorOfNumber(2));
        number->setOpacity(0);
        number->setPosition(_coordinate->PositionOfNumber(2));
        this->addChild(number,6,GOLD_NUM_INSERT_NUMBER);
    
        int xoffset = number->getContentSize().width;
        
        auto minus=Sprite::create("-.png");
        minus->setAnchorPoint(_coordinate->AnchorOfSign(2));
        minus->setPosition(_coordinate->PositionOfSign(2,xoffset));
        minus->setOpacity(0);
        this->addChild(minus,6,GOLD_NUM_INSERT_JIANHAO);
    
        auto plus=Sprite::create("+.png");
        plus->setAnchorPoint(_coordinate->AnchorOfSign(2));
        plus->setPosition(_coordinate->PositionOfSign(2,xoffset));
        plus->setOpacity(0);
        this->addChild(plus,6,GOLD_NUM_INSERT_JIAHAO);
    
        auto jinbi=Sprite::create("jinbi-game.png");
        jinbi->setAnchorPoint(_coordinate->AnchorOfGold(2));
        jinbi->setOpacity(0);
        jinbi->setPosition(_coordinate->PositionOfGold(2,xoffset));
        this->addChild(jinbi,6,GOLD_NUM_INSERT_JINBI);
        ...
        if( gold!=0 ) {
            jinbi->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfGold(2,xoffset)),NULL),
                ScaleTo::create(0,0),NULL));
            number->runAction(Sequence::create(
                DelayTime::create(0),
                Spawn::create(FadeIn::create(1.5),
                MoveTo::create(1.5,_coordinate->DestOfNumber(2)),NULL),
                ScaleTo::create(0,0),NULL));
            if(gold>0) {
                plus->runAction(Sequence::create(
                    DelayTime::create(0),
                    Spawn::create(FadeIn::create(1.5),
                    MoveTo::create(1.5,_coordinate->DestOfSign(2,xoffset)),NULL),
                    ScaleTo::create(0,0),NULL));
            } else {
                minus->runAction(Sequence::create(
                    DelayTime::create(0),
